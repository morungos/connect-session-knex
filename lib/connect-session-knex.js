...
module.exports = function(connect, sessionOptions) {
...
function KnexStore(options) {
	...
	this.transact = function(knex, handler) {
		if (sessionOptions.transactional) {
...
KnexStore.prototype.get = function(sid, fn) {
	...
	return that.ready.then(function () {
		return that.transact(that.knex, function (trx) {
			return trx
				.select('sess')
				.from(that.tablename)
				.where('sid', '=', sid)
...
KnexStore.prototype.set = function(sid, sess, fn) {
	...
	if (that.knex.client.dialect === 'sqlite3') {
		...
		return that.ready.then(function () {
			return that.transact(that.knex, function (trx) {
				return trx.raw(sqlitefastq, [sid, new Date(expired).toISOString(), sess ])
		...
		return that.ready.then(function () {
			return that.transact(that.knex, function (trx) {
				return trx.raw(postgresfastq, [sid, new Date(expired).toISOString(), sess ])
				.then(function (result) {
					if (fn) {
						fn(null, result);
					}
		...
		return that.ready.then(function () {
			return that.transact(that.knex, function (trx) {
				return trx.raw(mysqlfastq, [sid, new Date(expired).toISOString(), sess ])
				.then(function (result) {
					if (fn) {
						fn(null, result);
					}
